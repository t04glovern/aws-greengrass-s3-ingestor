...
class TestBatchJSONMessageProcessor(unittest.TestCase):
    def test_process_messages(self):
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            mock_messages = [
                Message(stream_name='stream1', sequence_number=i, ingest_time=1000, 
                        payload=bytes('{"message": "' + m + '"}', 'utf-8')) 
                for i, m in enumerate(['message1', 'message2', 'message3'], start=1)
            ]
            invalid_json_message = Message(stream_name='stream1', sequence_number=1, ingest_time=1000, payload=bytes('test', 'utf-8'))
            ...
            self.assertTrue(os.path.exists(os.path.join(tmpdirname, "batch_0.jsonl.gz")))

            # Testing with zero messages - no new file should be created
            read_messages_mock.return_value = []
            loop.run_until_complete(bmp.Run(under_test=True))
            self.assertFalse(os.path.exists(os.path.join(tmpdirname, "batch_1.jsonl.gz")))
