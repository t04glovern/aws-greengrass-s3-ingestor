...
class TestDirectoryUploader(unittest.TestCase):
    ...
    def test_ProcessStatus(self):
        ...
        du = DirectoryUploader(tmpdir+"/*.csv","test-bucket","sample",1,logger=logger,client=mock_client)
        loop = asyncio.get_event_loop()
    ...
    def test_scan_dirnotexist(self):
        ...
        du = DirectoryUploader(fakedir,"test-bucket","sample",1,logger=mock_logger,client=mock_client)
        loop = asyncio.get_event_loop()
        loop.run_until_complete(du._DirectoryUploader__scan(under_test=True)) 
        mock_client.assert_not_called()
    ...
    def test_wrongpath(self):
        ...
        du = DirectoryUploader(tmpdir+"/test*/test1.csv","test-bucket","sample",1,logger=mock_logger,client=mock_client)
        loop = asyncio.get_event_loop()
        loop.run_until_complete(du._DirectoryUploader__scan(under_test=True)) 
        mock_client.assert_not_called()
        mock_error.assert_called_once()
