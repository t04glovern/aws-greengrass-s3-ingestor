...
class DirectoryUploader:
    ...
    def __init__(self, pathname, bucket_name, prefix, interval, logger: logging.Logger, client: StreamManagerClient = None):
        ...
        try:
            self.__client.delete_message_stream(
                stream_name=self.__status_stream_name)
    ...
    async def __processStatus(self, under_test=False):
        ...
        while keep_looping:
            try:
                ...
                for message in messages_list:
                    ...
                    if status_message.status == Status.Success:
                        ...
                        self.__filesProcessed.remove(
                            file_url.partition("file://")[2])
            ...
            self.__logger.info(
                f"Sleeping for {self.__status_interval} seconds")
            await asyncio.sleep(self.__status_interval)
            keep_looping = not under_test

    async def Run(self):
        tasks = [asyncio.create_task(
            self.__scan()), asyncio.create_task(self.__processStatus())]
